...
- **저장된 모델 불러와서 추론**
  - 해당 부분이 조금 특이함
    ...
    **.SA/analysis/data_eda.ipynb** 를 통해 학습한 pkl 파일을 추론에 사용함 **<code><span style="color:orange">사전에 학습된 파라미터가 필요했기 때문</span></code>**
...
### 📄 `__init__.py`
- `model` 디렉토리를 Python 패키지로 인식하도록 설정(안쓰면 모듈이 인식이 안됨..이번에 이 존재에 대해 깨달음).
